...
void corr_mom(int N, Real* mom)
{
	...
	if (i < N){
        ...
        if(gamma_gamma == Real(0.0) ){
            ...
            mom[ParticleData<Real>::idx(i,0)] = cc*mec<Real>;
            mom[ParticleData<Real>::idx(i,1)] = cc*mec<Real>;
            mom[ParticleData<Real>::idx(i,2)] = cc*mec<Real>;
        }
        else if (gamma_gamma < Real(2.0)){
            const auto cc = bb/gamma_gamma;
            mom[ParticleData<Real>::idx(i,0)] *= cc;
            mom[ParticleData<Real>::idx(i,1)] *= cc;
            mom[ParticleData<Real>::idx(i,2)] *= cc;
        }
    }
}
...
{
    const auto N = pdata.num_particles;
    corr_mom<<<(N+255)/256, 256>>>(N, pdata.m_momentum);
}
