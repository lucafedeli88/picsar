...
template <typename RealType, typename VectorType>
auto generate_alt_pair_table(RealType chi_min, RealType chi_max, int chi_size, int frac_size)
{
    std::cout << "Preparing alt pair production table (always in double precision) [" << typeid(RealType).name() << ", " << chi_size << " x " << frac_size <<"]...\n";
    std::cout.flush();
...
void do_test(const std::vector<part<double>>& data)
{
    ...
    const auto alt_pair_table =
       generate_alt_pair_table<RealType,ThrustDeviceWrapper<RealType>>(
            table_chi_min,
            table_chi_max,
            table_chi_size,
            table_frac_size);
